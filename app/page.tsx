  ...
  const handleConvert = async (e) => {
    ...
    try {
      const res = await fetch('/api/convert', {
        ...
        body: JSON.stringify({ rawUrl: inputUrl.trim(), userId: user.id })
      });
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div>
            {user ? (
              <a
                href="/profile"
                className="flex items-center gap-1.5 bg-white text-orange-600 font-bold px-4 py-2 rounded-lg text-sm shadow hover:bg-orange-50 transition"
              >
      ...
      <section className="bg-gradient-to-br from-orange-500 via-orange-600 to-red-600 text-white py-12 px-4 shadow-inner">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-orange-100 text-sm sm:text-base mb-6">
            Mua sắm qua link hoàn tiền và rút số dư trực tiếp về tài khoản ngân hàng của bạn
          </p>
          ...
          <form onSubmit={handleConvert} className="flex flex-col sm:flex-row gap-2 max-w-2xl mx-auto">
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-yellow-400 hover:bg-yellow-500 text-slate-950 font-bold px-6 py-3.5 rounded-xl text-sm shadow flex items-center justify-center gap-2 shrink-0 transition disabled:opacity-70"
            >
          ...
          {convertedUrl && (
            <div className="mt-6 p-4 bg-white/10 rounded-xl backdrop-blur-md border border-white/20 flex flex-col sm:flex-row items-center justify-between gap-3 text-left max-w-2xl mx-auto">
              <div className="overflow-hidden w-full">
                <p className="text-xs text-orange-200 font-medium">Link đã được gắn mã tích lũy hoàn tiền:</p>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-white text-orange-600 font-bold px-5 py-2.5 rounded-lg text-sm flex items-center gap-1.5 shrink-0 hover:bg-orange-50 shadow transition"
              >
