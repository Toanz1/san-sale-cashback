...
// Chuỗi Cookie trích xuất từ phiên đăng nhập Shopee Affiliate
...
// 1. Giải mã link rút gọn (s.shopee.vn, shp.ee, vt.tiktok.com) để lấy URL gốc
...
export async function POST(req) {
  try {
    ...
    // Tự động mở link rút gọn nếu có
    ...
    // ==========================================
    // TRƯỜNG HỢP 1: SHOPEE -> GỌI TRỰC TIẾP GRAPHQL CỦA SHOPEE
    // ==========================================
    if (platform === 'Shopee') {
      ...
      if (!affiliateUrl) {
        console.error('Shopee Direct API Error:', JSON.stringify(resJson));
        return NextResponse.json(
          {
            error:
              'Không thể tạo link hoàn tiền Shopee. Phiên đăng nhập Shopee có thể đã hết hạn hoặc cookie không hợp lệ.',
          },
          { status: 500 }
        );
      }
