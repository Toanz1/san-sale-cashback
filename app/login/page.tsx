...
function LoginForm() {
  ...
  // Lưu trữ mã giới thiệu
  const [refCode, setRefCode] = useState<string | null>(null);

  useEffect(() => {
    // 1. Kiểm tra tham số ref trên link (VD: /login?ref=UID123456)
  ...
  // Hàm đồng bộ và lưu thông tin User vào bảng profiles
    ...
    try {
      ...
      if (!existingProfile) {
        ...
        await supabase.from('profiles').insert([{
          ...
          referred_by: referrer ? referrer.trim().toUpperCase() : null,
          balance: 0
        }]);
      } else {
